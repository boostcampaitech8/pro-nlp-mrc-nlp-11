...
def postprocess_qa_predictions(
    ...
    for example_index, example in enumerate(tqdm(examples)):
        ...
        for pred in predictions:
            ...
            answer_text = context[offsets[0] : offsets[1]]
            # Debug: Check if answer contains quotes
            logger.debug(f"Before normalization: {repr(answer_text)}")
            # Normalize multiple quotation marks to single quotation mark
            answer_text = re.sub(r'"{2,}', '"', answer_text)
            logger.debug(f"After normalization:  {repr(answer_text)}")
            pred["text"] = answer_text
